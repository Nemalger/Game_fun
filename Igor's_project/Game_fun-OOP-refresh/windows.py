...
class PlanetData:
    ...
    physonium: int = 0
    food_factory: int =0
    energy_factory: int = 0
    materials_factory: int = 0
    physonium_factory: int = 0
    # Зависимости Германа:

    def update(self, dt):
        self.humans += max(1, int(self.dp_dt(self.humans) * dt))
        self.food += max(1, int(self.dp_dt(self.food) * dt))
        self.energy += max(1, int(self.dp_dt(self.energy) * dt))
        self.materials += max(1, int(self.dp_dt(self.materials) * dt))
        self.physonium += max(1, int(self.dp_dt(self.physonium) * dt))
...
class DropBut(Button):
    def __init__(self, **kwargs):
        ...
        types = ['Food', 'Materials', 'People', 'Energy', 'Physonium']
...
class WindowManager(ScreenManager):
    ...
    #     self.planet_data.planet_1.update(dt)
    #     self.planet_data.planet_2.update(dt)
    #     self.planet_data.planet_3.update(dt)
